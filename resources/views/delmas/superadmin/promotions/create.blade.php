...
@section('content')
    ...
    <form action="{{ route('superadmin.promotions.store') }}" method="POST">
        @csrf
        <div class="flex flex-col sm:flex-row justify-between w-full mb-4">
            <label class="block text-sm w-full mb-4 sm:mb-0 sm:w-2/3 sm:mr-3">
                ...
                <input class="@error('name') border-red-500 @enderror block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" value="{{ old('content') }}" name="name" placeholder="Exemple : SIO 1">
                @error('name')
