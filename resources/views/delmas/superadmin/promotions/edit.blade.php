...
@section('content')
    ...
    <form action="{{ route('superadmin.promotions.update', $promotion) }}" method="POST">
        @csrf
        <div class="flex flex-col sm:flex-row justify-between w-full mb-4">
            <label class="block text-sm w-full mb-4 sm:mb-0 sm:w-2/3 sm:mr-3">
                <span class="text-gray-700 dark:text-gray-400">Nom de la promotion</span>
                <input class="@error('name') border-red-500 @enderror block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" value="{{ $promotion->name }}" name="name" placeholder="Exemple : SIO1">
                @error('name')
            ...
            <label class="block text-sm w-full sm:w-1/3">
                ...
                <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" name="serie_id">
                    @foreach($series as $serie)
                        <option value="{{ $serie->getKey() }}" @if($serie->getKey() == $promotion->serie->getKey()) selected @endif>{{ $serie->name }}</option>
                    @endforeach
                </select>
            </label>
        </div>
