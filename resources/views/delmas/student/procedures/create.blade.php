...
@section('content')
    ...
    <form x-data="{ resend: false }" action="{{ route('student.procedures.store') }}" method="POST">
        @csrf
        <livewire:student.create-procedure />
        ...
        <div class="flex flex-col mt-4 sm:flex-row justify-between w-full">
            ...
            <label class="block text-sm mb-4 sm:mb-0 w-full sm:w-1/2">
                ...
                <select x-on:change="$event.target.value === '0' ? resend = false : resend = true" class="@error('resend') border-red-500 @enderror block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" name="resend">
