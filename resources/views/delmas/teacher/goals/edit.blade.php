...
@section('content')
    ...
    <form action="{{ route('teacher.goals.update', $goal) }}" method="POST">
        @csrf
        ...
        <div class="flex flex-col sm:flex-row justify-between w-full mb-4">
            <label class="block text-sm w-full mb-4 sm:mb-0 sm:w-2/3 sm:mr-3">
                ...
                <input class="@error('content') border-red-500 @enderror block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" value="{{ $goal->content }}" name="content" placeholder="Objectif : faire 5 demandes avant Noël, 24 Décembre 2022">
                @error('content')
            ...
            <label class="block text-sm w-full sm:w-1/3">
                ...
                <select class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray" name="promotion_id">
                    @foreach($promotions as $promotion)
                        <option value="{{ $promotion->getKey() }}" @if($goal->promotion->getKey() == $promotion->getKey()) selected @endif>{{ $promotion->name }}</option>
                    @endforeach
                </select>
            </label>
        </div>
